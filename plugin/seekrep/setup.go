...
func seekrepParse(c *caddy.Controller) (*SeekrEPHandler, error) {
	var (
		seekrep *SeekrEPHandler
		err     error
	)
	i := 0
	for c.Next() {
...
func ParseStanza(c *caddy.Controller) (*SeekrEPHandler, error) {
	k8s := New([]string{""})
