...
func (s *SeekrEPHandler) nsAddrs(external, headless bool, zone string) []dns.RR {
	var (
		svcNames      []string
		svcIPs        []net.IP
		foundEndpoint bool
	...
	for _, localIP := range s.localIPs {
		endpoints := s.APIConn.EpIndexReverse(localIP.String())

		// Collect IPs for all Services of the Endpoints
		for _, endpoint := range endpoints {
			foundEndpoint = true
			svcs := s.APIConn.SvcIndex(endpoint.Index)
			for _, svc := range svcs {
				if external {
					svcName := strings.Join([]string{svc.Name, svc.Namespace, zone}, ".")

					if headless && svc.Headless() {
						for _, sb := range endpoint.Subsets {
							for _, a := range sb.Addresses {
								svcNames = append(svcNames, endpointHostname(a, s.endpointNameMode)+"."+svcName)
	...
	if !foundEndpoint {
		svcIPs = make([]net.IP, len(s.localIPs))
		svcNames = make([]string, len(s.localIPs))
		for i, localIP := range s.localIPs {
